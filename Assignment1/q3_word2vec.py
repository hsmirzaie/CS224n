...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x/=np.sqrt(np.sum(pow(x,2),axis=1,keepdims=True))
    ### END YOUR CODE
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    for k in range(K):
        ...
        z=sigmoid(np.dot(outputVectors[neg_sample].T,predicted))
        cost-=np.log(1.0-z)
        gradPred+=outputVectors[neg_sample]*z
        grad[neg_sample]+=predicted*z
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    predicted_ind=[]
    for word in contextWords:
        predicted_ind.append(tokens[word])
    predicted_vectors = inputVectors[predicted_ind]
