...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(window_size):
            raw_sentence.insert(0,start)
            raw_sentence.append(end)
            
        windows=[]
        for i in range(len(raw_sentence)-2*window_size):
            windows.append(raw_sentence[i:i+2*window_size+1])
            
        for i in range(len(windows)):
            feature_list=[]  
            for words in windows[i]:
                for features in words:
                    feature_list.append(features)
            windowed_data.append((feature_list,labels[i]))
 
        #or equivalently
        #for i in range(len(windows)):    
            #windowed_data.append(([features for words in windows[i] for features in words],labels[i]))
