...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        _sentence=sentence[:]
        _labels=labels[:]
        if len(_sentence)>max_length:
            _sentence, _labels = _sentence[:max_length], _labels[:max_length]
        else:
            while len(_sentence)<max_length:
                _sentence.append(zero_vector)
                _labels.append(zero_label)
        mask=[]
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable("b_2", [self.config.n_classes,],tf.float32,
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.boolean_mask(CE,self.mask_placeholder)
